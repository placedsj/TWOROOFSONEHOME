...
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children?: React.ReactNode }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200"
...
const TimelinePhase = ({ phase, title, time, status, onClick }: { phase: string, title: string, time: string, status: 'completed' | 'current' | 'future', onClick: () => void }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left relative flex-1 p-4 rounded-lg border-2 cursor-pointer transition-all hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:ring-offset-2 ${colors[status]} group`}
    >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sections.map((section) => (
              <button
                ...
                className="w-full text-left bg-white p-6 rounded-xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer border border-slate-100 group focus:outline-none focus:ring-2 focus:ring-royal-500 focus:ring-offset-2"
              >
